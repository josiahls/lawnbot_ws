        ...
        for i in range(0, len(self.ranges), 2):
            ...
            if (self.max_range > np.abs(adjust_x) and
                ...
                if (self.expand_state(plot_y, plot_x)):
                    self.state[plot_y + (self.y_offset - previous_offset_y)][
                        plot_x - (self.x_offset - previous_offset_x)] = 2
                    # sleep(.5)
        ...
        if not State.lock:
            ...
            if not State.lock:
                while self.state.shape[0] <= y:
                    State.lock = True
                    rospy.loginfo("State:expand_state: Expanding in the y:%s shape: %s direction by %s", y,
                                  self.state.shape, y - self.state.shape[0])
